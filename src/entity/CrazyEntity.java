...
public class CrazyEntity extends Entity {
	...
	final static double DEATH_SPEED = 40;
	
	final static double ACCEL_FACTOR = 0.01;
	...
	public void update() {
		...
		velocity.setX(velocity.getX() + (Math.random() / RANDOM_FACTOR - (1 / (2 * RANDOM_FACTOR))) * (1 + timeAlive * ACCEL_FACTOR));
		velocity.setY(velocity.getY() + (Math.random() / RANDOM_FACTOR - (1 / (2 * RANDOM_FACTOR))) * (1 + timeAlive * ACCEL_FACTOR));
		timeAlive++;
	...
	private void resetPosition() {
		setPos(new Position(START_LEFT_BOUND + Math.random() * (START_RIGHT_BOUND - START_LEFT_BOUND), 
				START_UPPER_BOUND + Math.random() * (START_UPPER_BOUND - START_LOWER_BOUND)));
	}
